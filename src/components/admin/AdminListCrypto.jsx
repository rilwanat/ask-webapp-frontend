...
// import WidgetCryptos from '../widgets/WidgetCryptos';
...
export default function AdminListCrypto({ 
  isMobile,
    currentRequestSlide, carouselRequestItems, setCurrentRequestSlide,
    currentBeneficiarySlide, carouselBeneficiaryItems, setCurrentBeneficiarySlide,
    currentcryptoslide, carouselSponsorItems, setCurrentcryptoslide,
...
useEffect(() => { 
  ...
  const [loading, setLoading] = useState(true);
    ...
    const handleCryptoRowClick = (crypto, e) => {
      {                
          // alert(JSON.stringify(crypto, null, 2));
  
          // const encryptedData = AES.encrypt(JSON.stringify(product), 'encryptionKey').toString();
          navigate(`/specific-crypto/${crypto.id}`, {
              state: {
                selectedCrypto: crypto, // Pass any data you want as props here
              }
            });
      ...
      const currentPageName = "Crypto";
    


      const [isDataloading, setIsDataLoading] = useState(true);
      const [cryptos, setCryptosData] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
    const [itemsPerPage] = useState(10);
    const [totalItems, setTotalItems] = useState(cryptos ? cryptos.length : 0);
    ...
    || crypto.address.toLowerCase().includes(searchQuery.toLowerCase()) 
    // || crypto.type.toLowerCase().includes(searchQuery.toLowerCase()) 
    // || crypto.tags && crypto.tags.some(tag => tag.toLowerCase().includes(searchQuery.toLowerCase())) 
    // || crypto.categories && crypto.categories.some(category => category.toLowerCase().includes(searchQuery.toLowerCase()))
    // || crypto.price.includes(searchQuery)
);

const totalFilteredItems = filteredCryptos.length;
...
const currentfilteredCryptos = filteredCryptos.slice(
...
let countFiltered = indexOfFirstFilteredItem + 1;
  ...
  const handleData = async () => {
    ...
    try {
      ...
      // alert(JSON.stringify(adminCryptosResponse.data.data), null, 2);
      setCryptosData(adminCryptosResponse.data.data);  // Update state with  count
  
  
      // openNotificationModal(true, currentPageName, "");
      // alert(JSON.stringify(adminCryptosResponse.data.data), null, 2);  // Update state with cryptos count
...
<div className='flex flex-col items-center px-0 sm:px-16 md:px-24 h-full'>
  ...
  <div 
    ...
    <div 
              onClick={() =>  navigateTo('/admin-add-crypto')}
              style={{ width: '200px' }}
                ...
                <div className="inline-block min-w-full py-2 ">
                                                        ...
                                                        <div className='overflow-x-auto mt-4'>
                  {
                    ...
                    <tbody className='text-xs '>
                      {currentfilteredCryptos.map((crypto, index) => (
                          <tr key={crypto.id} className={index % 2 === 0 ? 'bg-white' : 'bg-softTheme'}
                        onClick={(e) => handleCryptoRowClick(crypto, e)} 
                          style={{ cursor: "pointer" }}
                          >
                          <td className='px-2 py-4 whitespace-no-wrap border-b border-gray-200'>
                          {countFiltered++}
                          </td>
                          <td className="px-2 py-4 whitespace-no-wrap border-b border-gray-200 cursor-pointer" 
                            onClick={(e) => {
                            }}>
                            {crypto.network}
                          </td>
