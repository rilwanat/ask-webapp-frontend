...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $helpToken = $_POST['help_token'] ?? null;
...
// }
    ...
    $authResult = $response->checkIfUserCanUpdateHelpRequest($email);
    ...
    // if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) 
    {
        ...
        // if (move_uploaded_file($_FILES['image']['tmp_name'], $uniqueTargetFilePath)) 
        {
            ...
            if ($email && $description) {
                ...
                if ($response->UpdateHelpRequest($email, $description, $helpToken)) {
                    http_response_code(200);
                    echo json_encode(["status" => true, "message" => "Help Request updated successfully."]);
                } else {
                    http_response_code(500);
                    echo json_encode(["status" => false, "message" => "Unable to update your Help Request."]);
                }
            } else {
                http_response_code(400);
