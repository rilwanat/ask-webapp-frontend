...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($data->email) && !empty($data->verificationCode)) {
        ...
        $codeValid = $response->checkIfEmailCodeIsValidForMobilePasswordReset($data->email, $data->verificationCode);
        ...
        if ($codeValid) {
            ...
            echo json_encode(array("status" => true, "message" => "Reset code verification successful."
            // , "userData" => $userData
        ));
        } else {
            // User authentication failed
            http_response_code(400);
            echo json_encode(array("status" => false, "message" => "Reset code verification failed.", 
        ...
        http_response_code(400);
        echo json_encode(array("status" => false, "message" => "Reset code validation data is incomplete."));
    }
}
?>
